...
extern DWORD CallType HK_SendSingleCommand(const char* commandString)
{
	INPUT inputToSend = GetInputStructFromString(commandString);
...
//=============================================================================
//
// Gamepad input functions and data
//
//-----------------------------------------------------------------------------
static std::map<DWORD, Gamepad*> GamepadIdToPad;

extern DWORD CallType PAD_NewGamepad(DWORD GamepadId)
{
	bool found = (GamepadIdToPad.find(GamepadId) != GamepadIdToPad.end());
	if (found)
	{
		return 0;
	}

	GamepadIdToPad[GamepadId] = new Gamepad(GamepadId);
	return GamepadId;
}

extern void CallType PAD_Update(DWORD GamepadId)
{
	auto it = GamepadIdToPad.find(GamepadId);
	if (it != GamepadIdToPad.end())
	{
		it->second->Update();
	}
}

extern void CallType PAD_Refresh(DWORD GamepadId)
{
	auto it = GamepadIdToPad.find(GamepadId);
	if (it != GamepadIdToPad.end())
	{
...
MouseHookDLL_API GamepadState CallType PAD_GetGamepadState(DWORD GamepadId)
{
	...
	{
		auto pad = it->second;
		data.LeftStickX = pad->LeftStick_X();
		data.LeftStickY = pad->LeftStick_Y();
		data.RightStickX = pad->RightStick_X();
		data.RightStickY = pad->RightStick_Y();
		data.LeftTrigger = pad->LeftTrigger();
		data.RightTrigger = pad->RightTrigger();
		data.ButtonStateFlags = pad->GetAllButtonStateFlags();
	}
	return data;
}
